...
class PizzaExistsValidator:
    def __init__(self, order: models.OrderBase):
        self.order = order

    def validation(self, db: Session):
        if not all(
            get_pizza(db, ordered_pizza.pizza.name) != None
...
class StatusValidator:
    def __init__(self, orderStatus: models.OrderHistoryBase):
    ...
    def validation(self, db: Session):
        ...
        history = (
            db.query(models.OrderHistory).filter(
        ...
        if not history:
            if _status > 1:
                return WrongStatusException("First status must be VERIFICATION")
        else:
            statuses = [obj.status for obj in history]
            if _status in statuses:
                return WrongStatusException("This status is already implemented")
            if (_status - max(statuses)) > 1:
                return WrongStatusException("Wrong status order")
