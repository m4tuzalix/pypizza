...
from tests.order_fixtures import (
    pytest,
    client,
    db_order,
    db_pizza,
...
def test_create_incorrect_order_with_empty_pizzas(
    test_db, db_pizza, incorrect_order_empty_pizzas
):
...
def test_order_updatestatus_using_correct_ordering(test_db, db_order, status_id):
    for id in status_id:
        response = client.post(
            f"/order/update_status/", json={"order_id": db_order, "status": id}
        )
...
def test_order_updatestatus_using_incorrect_ordering(test_db, db_order):
    response = client.post(
        f"/order/update_status/", json={"order_id": db_order, "status": 1}
    )
    assert response.json()["order_id"] == 1
    assert response.json()["status"] == Status(1).name
    response = client.post(
        f"/order/update_status/", json={"order_id": db_order, "status": 2}
    )
    assert response.json()["order_id"] == 1
    assert response.json()["status"] == Status(2).name
    response = client.post(
        f"/order/update_status/", json={"order_id": db_order, "status": 4}
    )
